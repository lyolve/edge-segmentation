...
/ *增加对比度 * /
    void
    addContrast(Mat &srcImg);
/ *交换两个Mat * /
    void
    swapMat(Mat &srcImg, Mat &dstImg);
/ *二值化图像。0->0, 非0->255 * /
                         void
                         binaryzation(Mat &srcImg);
/ *检测连通域，并删除不符合条件的连通域 * /
    void
    findConnectedDomain(Mat &srcImg, vector<vector<Point>> &connectedDomains, int area, int WHRatio);
/ *提取连通域的骨架 * /
    void
    thinImage(Mat &srcImg);
/ *获取图像中白点的数量 * /
    void
    getWhitePoints(Mat &srcImg, vector<Point> &domain);
/ *计算宽高信息的放置位置 * /
    Point
    calInfoPosition(int
                        imgRows,
                    ...
                    int
                        padding,
                    const std::vector<cv::Point> &domain);
...
{
    Mat
        srcImg = imread("./image/20180803215201452.jpg");
    Mat
        dstImg,
        dstImg2;
    // 灰度化
    ...
    Mat
        lookUpTable(1, 256, CV_8U, Scalar(0));
...
{
    Mat
        lookUpTable(1, 256, CV_8U, Scalar(255));
        ...
        for (int j = 1; j < (nl - 1); j++)
        {
            ...
            for (int i = 1; i < (nc - 1); i++)
            {
                if (data[i] == 255)
                {
                    ...
                    {
                        int ap = 0;
                        if ((neighbourhood[1] == 0) & &(neighbourhood[2] == 1))
                            ap++;
                        if ((neighbourhood[2] == 0) & &(neighbourhood[3] == 1))
                            ap++;
                        if ((neighbourhood[3] == 0) & &(neighbourhood[4] == 1))
                            ap++;
                        if ((neighbourhood[4] == 0) & &(neighbourhood[5] == 1))
                            ap++;
                        if ((neighbourhood[5] == 0) & &(neighbourhood[6] == 1))
                            ap++;
                        if ((neighbourhood[6] == 0) & &(neighbourhood[7] == 1))
                            ap++;
                        if ((neighbourhood[7] == 0) & &(neighbourhood[8] == 1))
                            ap++;
                        if ((neighbourhood[8] == 0) & &(neighbourhood[1] == 1))
                            ap++;
                        if (ap == 1)
                        {
                            if (inOddIterations & &(neighbourhood[3] * neighbourhood[5] * neighbourhood[7] == 0) & &(neighbourhood[1] * neighbourhood[3] * neighbourhood[5] == 0))
                            {
                                deleteList.push_back(Point(i, j));
                            }
                            else if (!inOddIterations & &(neighbourhood[1] * neighbourhood[5] * neighbourhood[7] == 0) & &(neighbourhood[1] * neighbourhood[3] * neighbourhood[7] == 0))
                            {
                                deleteList.push_back(Point(i, j));
                            }
                        }
